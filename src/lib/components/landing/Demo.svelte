<script>
  ...
  const statusConfig = {
    ...
    'in-progress': { label: 'In Progress', bg: 'bg-amber-100', text: 'text-amber-800', dot: 'bg-amber-600' },
    planned: { label: 'Planned', bg: 'bg-gray-100', text: 'text-gray-600', dot: 'bg-gray-400' }
  };

  function groupedByStatus() {
    const groups = { planned: [], 'in-progress': [], shipped: [] };
  ...
  <div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-8">
      ...
      <div class="flex bg-clay rounded-md p-1">
        <button
          class="px-4 py-1.5 text-sm font-medium rounded-sm transition-all cursor-pointer {view === 'timeline' ? 'bg-white text-ink shadow-sm' : 'text-ink-muted hover:text-ink-secondary'}"
          onclick={() => view = 'timeline'}
        >
          Timeline
        </button>
        <button
          class="px-4 py-1.5 text-sm font-medium rounded-sm transition-all cursor-pointer {view === 'kanban' ? 'bg-white text-ink shadow-sm' : 'text-ink-muted hover:text-ink-secondary'}"
          onclick={() => view = 'kanban'}
        >
          Kanban
        </button>
    ...
    <div class="bg-white rounded-lg border border-clay-border overflow-hidden">
            ...
            <div class="p-5 hover:bg-paper transition-colors">
              <div class="flex items-start gap-4">
                <div class="flex-shrink-0 mt-1">
                  <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {statusConfig[item.status].bg} {statusConfig[item.status].text}">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex items-center gap-5 text-sm text-ink-muted">
                    ...
                    <span class="inline-flex items-center gap-1.5">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-3.582 8-8 8a8.013 8.013 0 01-5.45-2.125L3 21l1.125-4.55A8.013 8.013 0 013 12c0-4.418 3.582-8 8-8s8 3.582 8 8z"/></svg>
                      {item.comments}
                    </span>
      ...
      {:else}
        <div class="p-5 grid grid-cols-1 md:grid-cols-3 gap-5">
          {#each Object.entries(groupedByStatus()) as [status, groupItems]}
            {@const cfg = statusConfig[status]}
            <div>
              <div class="flex items-center gap-2 mb-4">
                <span class="w-2 h-2 rounded-full {cfg.dot}"></span>
                <span class="text-sm font-semibold {cfg.text}">{cfg.label}</span>
                <span class="text-xs text-ink-muted bg-clay px-2 py-0.5 rounded-full">{groupItems.length}</span>
              </div>
              <div class="space-y-3">
                {#each groupItems as item}
                  <div class="bg-paper rounded-md p-4 border border-clay-border hover:border-clay transition-colors">
                    ...
                    <div class="flex items-center gap-4 text-xs text-ink-muted">
                      ...
                      <span class="inline-flex items-center gap-1">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-3.582 8-8 8a8.013 8.013 0 01-5.45-2.125L3 21l1.125-4.55A8.013 8.013 0 013 12c0-4.418 3.582-8 8-8s8 3.582 8 8z"/></svg>
                        {item.comments}
                      </span>
                    </div>
                  </div>
    ...
    <p class="text-center text-sm text-ink-muted mt-6">
      This roadmap is powered by <span class="font-medium text-ink-secondary">pforge</span> — try upvoting or switching views above.
    </p>
  </div>
</section>
